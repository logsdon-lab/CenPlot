...
def read_bed9(infile: str, *, chrom: str | None = None) -> pl.DataFrame:
    ...
    df_adj = adj_by_ctg_coords(df, "chrom")
    if chrom:
...
def read_bed_identity(infile: str, *, chrom: str | None = None):
    ...
    df_tri = (
        ...
        .select("query_name", "new_x", "new_y", "color", "group")
        # arr to new rows
        .explode("new_x", "new_y")
...
def standardize_coords(
    ...
    for track in dfs_track:
        ...
        new_dfs_track.append(Track(track.name, track.pos, track.opt, track.prop, df_adj_track))
...
def plot_one_cen(
    ...
    fig, axes = plt.subplots(
        ...
        len(track_info),
        1,
        # TODO: This should scale with the number of tracks?
        # figsize=(),
    ...
    # Get HOR axis.
    hor_track_idx = track_indices["hor"]
    ax: matplotlib.axes.Axes = axes[hor_track_idx]
    ...
    for row in df_stv.iter_rows(named=True):
        ...
        rect = mpatches.Rectangle((start, 0), end + 1 - start, 12, color=color, lw=0)
        ax.add_patch(rect)
    ...
    # Remove xyticks and yticklabels
    # ax.set_xticks([])
    ax.set_yticks([], [])
    ...
    for track in dfs_track:
        if track.pos == TrackPosition.Overlap:
            ...
            hor_track_idx = track_indices[track.name]
            track_ax: matplotlib.axes.Axes = axes[hor_track_idx]
        ...
        if track.opt == TrackOption.Label:
            ...
            for row in track.data.iter_rows(named=True):
                ...
                rect = mpatches.Rectangle(
                    (start, 0), end + 1 - start, 12, color=color, lw=0
                )
                track_ax.add_patch(rect)
        elif track.opt == TrackOption.SelfIdent:
            # Plot each diamond
            import plotnine as pln

            pfig = (
                pln.ggplot(track.data) +
                pln.geom_polygon(
                    mapping = pln.aes(x = "x", y = "y", fill = "color", group = "group"),
                )
            )
            pfig._draw_using_figure(fig, axs=[track_ax])
            ...
            # Add line
            track_ax.plot(track.data["chrom_st"], track.data["name"], color="black")
            # Fill in-between
            track_ax.fill_between(
                track.data["chrom_st"], track.data["name"], color="black"
            )
            # Trim plot to margins
            track_ax.margins(x=0, y=0)
            # remove spines
            for spine in ("right", "top"):
                track_ax.spines[spine].set_visible(False)
            # Limit spine range.
            # TODO: Remove ticks not within bounds.
            track_ax.spines["bottom"].set_bounds(0, track.data["chrom_end"].max())
...
def main():
    ...
    ap.add_argument(
        ...
        "--ht_prop",
        type=float,
        help="Height proportion of HOR track.",
        default=0.5,
    )
    ap.add_argument("-o", "--output")
    ap.add_argument("-d", "--output_dir")
    ap.add_argument(
        "-m",
        "--mer_order",
        default="large",
    ...
    STV_TRACK = Track("stv", TrackPosition.Center, TrackOption.Label, args.ht_prop, df_stv)
    ...
    *_, df_track_max_end = max(
    ...
    plots = [
        plot_one_cen(
            df_stv.filter(pl.col("chrom") == chrom),
            df_stv_ort.filter(pl.col("chrom") == chrom),
            [Track(trk.name, trk.pos, trk.opt, trk.prop, trk.data.filter(pl.col("chrom") == chrom)) for trk in dfs_track],
            chrom,
            min_st_pos,
            max_end_pos,
            args.ht_prop,
